...
## ISS-012 [cleanup] [P3] [open]

**`resolve_graph_ctx` 变量名和注释仍假设 YAML**

`resolve_graph_ctx()` 中变量名为 `graph_yml`、注释写 "Find the graph.yml path"，但实际上现在可能返回 `graph.db` 路径。不影响逻辑（只取 parent dir），但会误导读者。

### 需要改的地方
- `resolve_graph_ctx()` 中 `graph_yml` → `graph_path`
- 注释 "Find the graph.yml path" → "Find the graph file path (graph.yml or graph.db)"
- `GraphContext` 的 `graph_yml` 字段 → `graph_file`（及所有引用）
- `resolve_graph_path()` 函数名和错误信息也只提 `graph.yml`，应覆盖 `graph.db`

### 文件
- `crates/gid-cli/src/main.rs`: `resolve_graph_ctx()`, `resolve_graph_path()`, `struct GraphContext`

### 关联
- ISS-009 (SQLite backend) 的遗留清理
...
## ISS-014 [improvement] [P1] [open]
**`gid infer` 聚类质量差 — 需要自动参数选择和后处理**
...
当前 `gid infer` 把 Infomap 原始输出直接暴露给用户，没有 post-processing。在 Claude Code（1,902 文件）上测试：

**flat 模式** (`min_community_size: 2`):
- 457 个 component — 过度碎片化
- 大量 1-file component（无意义）
- 标签全是目录名重复（80 个 `utils`、50 个 `components`）

**hierarchical 模式** (`min_community_size: 5`):
- 927 个 component — 更糟
- 嵌套深度达 10 层（`0.2.3.0.0.0.0.0.0.0`）
- 顶层 component 吞了 2359 个文件（超过总文件数，有重复计数）

**核心问题**: 用户不应该猜参数。不同规模的 repo 需要不同设置，这应该自动化。

### 修复方案

**1. 自动参数选择（基于图规模）**:

| 文件数 | min_community_size | 模式 | 层级限制 |
|---|---|---|---|
| < 50 | 2 | flat | - |
| 50-500 | 3 | flat | - |
| 500-2000 | 5 | hierarchical | top 2 层 |
| 2000+ | 8 | hierarchical | top 2 层 |

**2. 后处理 pipeline**:
- **合并同名相邻 component**: 10 个 `utils` → 按 Infomap flow 相近度合并
- **层级裁剪**: hierarchical 超过 N 层的自动 flatten 到 parent
- **覆盖验证**: ∑(component.files) == total_files（不重不漏）
- **标签去重**: 同名 component 加区分后缀（如 `utils/bash`, `utils/auth`）
- **过滤噪声**: 单文件 component 自动归入最近的 neighbor component

**3. 质量指标输出**:
- 聚类数量 / 文件数比例（目标：10-50 个 component for ~2000 files）
- 最大 component 占比（如果 >50% 文件在一个 component → 警告）
- 孤立文件比例

### 关联
- ISS-003 (Leiden 算法研究)
- ISS-013 (SQLite backend)
